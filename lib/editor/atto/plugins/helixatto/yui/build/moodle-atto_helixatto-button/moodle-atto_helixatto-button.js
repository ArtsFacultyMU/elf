...
var TEMPLATE = '' +
    '<form class=\"atto_form\">' +
        '<div id="{{elementid}}_{{innerform}}" class="mdl-align" style=\"height:{{height}}px;\">' +
            '<iframe id=\"medialiframe\" style=\"border:0px;margin:0px;background:#ffffff;width:100%;height:{{iheight}};\" src=\"{{iframesrc}}\" allow=\"{{allow}}\"></iframe>' +
            '<button id=\"medial_insert\" class=\"btn btn-secondary submit {{hidden}}\">{{get_string "insert" component}}</button>' +
        '</div>' +
    '</form>';
...
Y.namespace('M.atto_helixatto').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _displayDialogue: function(e) {
        ...
        var buttonform = this._getFormContent(hidden, iheight);
      ...
      * @private
      */
    _getFormContent: function(hidden, iheight) {
    ...
    _doInsert : function(e){
        ...
        var url = embedLaunchURL + "?type=16&l=" + preid;
        var html = "";
