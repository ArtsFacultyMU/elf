...
M.atto_recordrtc.audiomodule = {
    init: function(scope) {
        ...
        // Fix for MDL-62872 (allow GB values)
        if (scope.get('maxrecsize').match(/(\d+)G/)) {
            cm.maxUploadSize = window.parseInt(scope.get('maxrecsize').match(/(\d+)G/)[1], 10) * Math.pow(1024, 3);
        }
