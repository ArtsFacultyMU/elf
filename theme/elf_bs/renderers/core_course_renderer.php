...
class theme_elf_bs_core_course_renderer extends core_course_renderer {
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        $content .= html_writer::start_tag('div', array('class' => 'course-info-container'));
        // course name
        $coursename = $chelper->get_course_formatted_name($course);
        $coursenamelink = '<span>'.html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)),
                                            $coursename, array('class' => $course->visible ? '' : 'dimmed')).'</span>';
        $content .= html_writer::tag('h3', $coursenamelink, array('class' => 'coursename'));
        
		$cCategory = $DB->get_record('course_categories',array('id'=>$course->category));
		$pathIds = explode('/',$cCategory->path);
		if(isset($pathIds[1])) {
			$cat = $DB->get_record('course_categories',array('id'=>$pathIds[1]));
			$content .= '<div class="categorylink">' .html_writer::link(new moodle_url('/course/index.php', array('categoryid' => $cat->id)),$cat->name) . '</div>';
			if(isset($pathIds[2])) {
				$cat = $DB->get_record('course_categories',array('id'=>$pathIds[2]));
				$content .= html_writer::link(new moodle_url('/course/index.php', array('categoryid' => $cat->id)),$cat->name,array('class' => 'categorylink'));
			}
		}
		
        // If we display course in collapsed form but the course has summary or course contacts, display the link to the info page.
        
        $content .= html_writer::end_tag('div'); // .h3 course cat
        //
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        if ($course->has_course_contacts()) {
            ...
            foreach ($course->get_course_contacts() as $userid => $coursecontact) {
				if($first)
					$content .= html_writer::tag('li', $coursecontact['rolename'].':');
