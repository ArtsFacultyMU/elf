/**
 * Customise the dock for this theme. ELF DOCK
 *
...
function customise_dock_for_theme(dock) {
    ...
    dock.on('dock:initialised', function() {
        var navbar = Y.one('header.navbar'),
            ...
            newmargintop = null,
            diff = null,
			isFullHeader = (Y.one('body').getData('header')=="full")?true:false;
        if (navbar && navbarbtn && container) {
			if(isFullHeader) {
				margintop = 104 - window.scrollY;
			} 
            diff = parseInt(navbar.get('offsetHeight'), 10);
